...
def employee_dashboard():
    ...
    # Fetch ALL requests so both the pending column and approved column can see them
    bookings = Request.query.all()
    return render_template("employee_dashboard.html", requests=bookings)
...
def delete_request(request_id):
    ...
    # Allow the original customer OR an employee/admin to delete the request
    if req.customer_id != current_user.id and current_user.role not in ["employee", "admin"]:
    ...
    # Redirect the user back to whichever dashboard they came from
